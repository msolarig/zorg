...
const engine_data = @import("../../../engine/data.zig");
const sql_wrap = engine_data.sql_wrap;
...
const Theme = struct {
    ...
    const fg_code = vaxis.Color{ .index = 66 }; // muted blue for code
    const fg_line_num = vaxis.Color{ .index = 240 }; // dark gray for line numbers
};
...
pub fn render(win: vaxis.Window, state: *State) void {
    ...
    const entry = state.currentEntry() orelse {
        printLine(win, 1, 1, "No selection", .{ .fg = Theme.fg_label, .dim = true });
        return;
    };

    var row: usize = 1; // Start after top border

    // Entry name - must duplicate to frame arena
    const entry_name = state.frameAlloc(entry.name) catch return;
    printLine(win, row, 1, entry_name, .{ .fg = Theme.fg_accent, .bold = true });
    row += 1;
...
fn previewWithBat(win: vaxis.Window, state: *State, path: []const u8, start_row: usize, max_lines: usize) bool {
    ...
    while (lines.next()) |line_raw| {
        ...
        printLine(win, row, 1, line, .{ .fg = Theme.fg_code }); // Inside border
        row += 1;
        line_num += 1;
    }
...
fn previewFileWithSyntax(win: vaxis.Window, state: *State, path: []const u8, start_row: usize, max_lines: usize) void {
    const file = std.fs.cwd().openFile(path, .{}) catch {
        printLine(win, start_row, 1, "(cannot read)", .{ .fg = Theme.fg_label, .dim = true });
        return;
    };
    defer file.close();
    ...
    if (read_size == 0) {
        printLine(win, start_row, 1, "(empty file)", .{ .fg = Theme.fg_label, .dim = true });
        return;
    }
    ...
    const content_buf = alloc.alloc(u8, read_size) catch {
        printLine(win, start_row, 1, "(memory error)", .{ .fg = Theme.fg_label, .dim = true });
        return;
    };
    ...
    while (display_line_num < end_line) {
        ...
        printLine(win, row, 1, line_num_final, .{ .fg = Theme.fg_line_num, .dim = true });
        ...
        if (use_syntax) {
            const tokens = if (is_zig)
                syntax.highlightZig(alloc, display_line) catch {
                    // Fallback to plain text on error
                    const plain = state.frameAlloc(display_line) catch break;
                    printLine(win, row, 1 + line_num_col_width, plain, .{ .fg = Theme.fg_code });
                    row += 1;
                    display_line_num += 1;
                    continue;
                }
            else if (is_json)
                syntax.highlightJson(alloc, display_line) catch {
                    const plain = state.frameAlloc(display_line) catch break;
                    printLine(win, row, 1 + line_num_col_width, plain, .{ .fg = Theme.fg_code });
                    row += 1;
                    display_line_num += 1;
...
fn previewDatabase(win: vaxis.Window, state: *State, path: []const u8, start_row: usize, max_lines: usize) void {
    const db_handle = sql_wrap.openDB(path) catch {
        printLine(win, start_row, 1, "(cannot open database)", .{ .fg = Theme.fg_label, .dim = true });
        return;
    };
    ...
    if (prepare != 0) {
        printLine(win, start_row, 1, "(query failed)", .{ .fg = Theme.fg_label, .dim = true });
        return;
    }
    ...
    while (sql_wrap.sqlite3_step(stmt.?) == 100) {
        ...
        printLine(win, row, 1, table_display, .{ .fg = Theme.fg_accent, .bold = true }); // Inside border
        
        const info = state.frameFmt(" {d} rows, {d} cols", .{ row_count, col_count }) catch break;
        printLine(win, row, 1 + table_name.len, info, .{ .fg = Theme.fg_value });
    ...
    if (table_count == 0) {
        printLine(win, start_row, 1, "(no tables)", .{ .fg = Theme.fg_label, .dim = true });
    }
}
...
fn previewDirectory(win: vaxis.Window, state: *State, path: []const u8, start_row: usize, max_lines: usize) void {
    ...
    while (iter.next() catch null) |entry| {
        ...
        if (count < max_lines - 1) {
            const icon: []const u8 = if (entry.kind == .directory) "d " else "f ";
            const color = if (entry.kind == .directory) Theme.fg_accent else Theme.fg_value;

            const name = state.frameAlloc(entry.name) catch break;

            printLine(win, row, 1, icon, .{ .fg = color }); // Inside border
            printLine(win, row, 3, name, .{ .fg = color });
            row += 1;
        }
        count += 1;
    }
    ...
    if (count >= max_lines) {
        if (state.frameFmt("... +{d} more", .{count - max_lines + 1})) |more| {
            printLine(win, row, 1, more, .{ .fg = Theme.fg_label, .dim = true }); // Inside border
        } else |_| {}
    }
}

fn formatSize(bytes: u64) struct { value: f64, unit: []const u8 } {
    if (bytes >= 1024 * 1024 * 1024) return .{ .value = @as(f64, @floatFromInt(bytes)) / (1024.0 * 1024.0 * 1024.0), .unit = "GB" };
    if (bytes >= 1024 * 1024) return .{ .value = @as(f64, @floatFromInt(bytes)) / (1024.0 * 1024.0), .unit = "MB" };
    if (bytes >= 1024) return .{ .value = @as(f64, @floatFromInt(bytes)) / 1024.0, .unit = "KB" };
