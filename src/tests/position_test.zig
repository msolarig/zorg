...
test "PositionManager buy fill initiates long exposure" {
    ...
    var pm = PositionManager.init();
    defer pm.deinit(alloc);

    const buy_fill = Fill.init(1, 1000, .Buy, 100.0, 50.0);
...
test "PositionManager sell fill initiates short exposure" {
    ...
    const sell_fill = Fill.init(1, 1000, .Sell, 100.0, 30.0);
...
test "PositionManager additional buy increases long exposure" {
    ...
    try pm.updateInstrumentExposure(alloc, Fill.init(1, 1000, .Buy, 100.0, 50.0));
    try pm.updateInstrumentExposure(alloc, Fill.init(2, 2000, .Buy, 101.0, 25.0));
...
test "PositionManager sell reduces long exposure" {
    ...
    try pm.updateInstrumentExposure(alloc, Fill.init(1, 1000, .Buy, 100.0, 50.0));
    try pm.updateInstrumentExposure(alloc, Fill.init(2, 2000, .Sell, 105.0, 20.0));
...
test "PositionManager sell flattens long exposure" {
    ...
    try pm.updateInstrumentExposure(alloc, Fill.init(1, 1000, .Buy, 100.0, 50.0));
    try pm.updateInstrumentExposure(alloc, Fill.init(2, 2000, .Sell, 105.0, 50.0));
