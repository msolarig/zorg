...
// Uses simple keyword matching for distribution-friendly highlighting
...
const Theme = struct {
    const fg_keyword = vaxis.Color{ .index = 66 }; // muted blue
    const fg_string = vaxis.Color{ .index = 65 }; // muted green
    const fg_comment = vaxis.Color{ .index = 240 }; // dark gray
    const fg_number = vaxis.Color{ .index = 137 }; // muted brown
    const fg_type = vaxis.Color{ .index = 127 }; // purple
    const fg_function = vaxis.Color{ .index = 214 }; // amber
    const fg_operator = vaxis.Color{ .index = 187 }; // beige
    const fg_normal = vaxis.Color{ .index = 187 }; // beige
};
