...
const Theme = struct {
    ...
    const fg_header = vaxis.Color{ .index = 66 }; // muted blue
...
pub fn render(win: vaxis.Window, state: *State) void {
    ...
    if (engine.track.size == 0) {
        printLine(win, 1, 1, "No data loaded", .{ .fg = Theme.fg_dim, .dim = true });
        return;
    }
    ...
    printLine(win, row, 1, header_display, .{ .fg = Theme.fg_header, .bold = true });
    row += 1;
    if (row >= max_h) return;
    ...
    while (i < show_first and row < max_h) : (i += 1) {
        ...
        printLine(win, row, 1, line_display, .{ .fg = Theme.fg_value });
        row += 1;
    }
    ...
    if (needs_separator and row < max_h) {
        ...
        const sep_col = if (max_w > sep_str.len) (max_w - sep_str.len) / 2 + 1 else 1;
        printLine(win, row, @intCast(sep_col), sep_str, .{ .fg = Theme.fg_dim, .dim = true });
        row += 1;
    }
    ...
    if (row < max_h) {
        ...
        printLine(win, row, @intCast(total_col), total_str, .{ .fg = Theme.fg_dim, .dim = true });
    }
}
