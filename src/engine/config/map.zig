...
pub const Map = struct {
    ...
    pub fn init(alloc: std.mem.Allocator, map_path: []const u8) !Map {
        ...
        const MapPlaceholder = struct { auto: []const u8, feed_mode: FeedMode, db: []const u8, table: []const u8, t0: u64, tn: u64, trail_size: u64, exec_mode: ExecMode, account: struct { balance: f64 }, output: struct { name: []const u8 } };
        ...
        map.account = Account{ .balance = parsed.value.account.balance };
        map.output = OutputConfig{ .name = try alloc.dupe(u8, parsed.value.output.name) };
    ...
    pub fn deinit(self: *Map) void {
        ...
        self.alloc.free(self.output.name);
    }
};
